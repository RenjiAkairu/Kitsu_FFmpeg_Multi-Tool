...
class FFmpegGUI(TkinterDnD_CTk):
    ...
    def _build_tab_video_converter(self, parent):
        ...
        ctk.CTkLabel(self.vid_card, text="").grid(row=5, column=0, pady=2)
    ...
    def update_vc_preset_options(self, _=None):
        ...
        if "CPU" in enc:
            ...
            self.vc_preset_tooltip.update_text("Graphics card processing speed levels:\nfrom p1 (fastest/coarsest image) to p7 (slowest/sharpest image)\n(Recommended: p4-p5)")
        else:
    ...
    def _build_ffmpeg_command_vc(self, input_file, output_file):
        ...
        if vc == "Copy (No re-encode)":
            ...
            if is_nvenc: cmd.extend(["-rc", "vbr", "-cq", str(crf), "-b:v", "0"])
            else: cmd.extend(["-crf", str(crf)])
